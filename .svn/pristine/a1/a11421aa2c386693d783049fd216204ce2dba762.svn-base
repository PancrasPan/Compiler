...
void AsmPrinter::EmitFunctionBody() {
 ...
 //bool ShouldPrintDebugScopes = MMI->hasDebugInfo();
  bool ShouldPrintDebugScopes = false;//ppp
  ...
  for (auto &MBB : *MF) {
    ...
    for (auto &MI : MBB) {
		//OutStreamer.AddComment(Twine(6));//ppp
  ...
  OutStreamer.AddComment("EmittedInsts:"+Twine(EmittedInsts));//ppp
  //OutStreamer.EmitRawText(StringRef("222222222222\n"));//ppp
...
void AsmPrinter::EmitBasicBlockStart(const MachineBasicBlock &MBB) const {
  ...
  if (isVerbose()) {
	...
	//DEBUG(dbgs() << MBB.getFullName() << " name machine basic block" << MBB.getNumber() << "\n");
	//DEBUG(dbgs() << "\n");
	//DEBUG(dbgs() << " is pred_emtpy" << MBB.pred_empty() << "isBlockOnlyReachableByFallthrough" << isBlockOnlyReachableByFallthrough(&MBB) << "\n");
